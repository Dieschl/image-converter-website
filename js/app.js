...
// Minimaler clientseitiger Bildkonverter
...
async function convertImage(file, targetType, quality, maxW){
  ...
  if (maxW && maxW > 0 && width > maxW){ height = Math.round(height * (maxW / width)); width = maxW; }
...
convertBtn.addEventListener('click', async () => {
  convertBtn.disabled = true; outputs = [];
  ...
  for (const f of files){
    ...
    } catch(e){ appendErrorCard(f, e); console.error(e); }
  }
...
function appendResultCard(origFile, blob, outName){
  ...
  card.innerHTML = `<div><strong>${escapeHtml(outName)}</strong></div>
...
function appendErrorCard(file, err){
  ...
  card.innerHTML = `<div><strong>${escapeHtml(file.name)}</strong></div>
    <div class="muted" style="color:#b00020;">Fehler: ${escapeHtml(err.message||'Konvertierung fehlgeschlagen')}</div>`;
  resultsEl.appendChild(card);
}

// Platzhalter: "Alle herunterladen" – kann mit JSZip ergänzt werden
DownloadAllBtn?.addEventListener?.('click', () => alert('Für ZIP-Download füge ich dir auf Wunsch JSZip hinzu.'));
